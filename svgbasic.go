...
// func getFilledState(path) {
// 	return path.style && path.style.includes(filled)
// }
	...
	if err == nil {
		if src.Wd > 0 && src.Ht > 0 {
			...
			for _, path := range src.Paths {
				if err == nil {
					// TODO get whether filled or not from style attribute in path segment
					// pass that boolean into pathParse function
					// fmt.Println("PAth style?", path.Style)
					// var isFilled = getFilledState(path)
					// segs, err = pathParse(path.D, isFilled)
